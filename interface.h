...
static bool is_show_right = TRUE;
...
void init_title_bar(){
	...
	gtk_label_set_markup(GTK_LABEL(close_label),"<span foreground='white' background='#B0D047' font_desc='20'>X</span>");
	//gtk_container_add(GTK_CONTAINER(event_box),close_label);
	g_signal_connect(G_OBJECT(event_box),"button-press-event",G_CALLBACK(gtk_main_quit),NULL);

	//*min_label;
	min_label= GTK_WIDGET(gtk_builder_get_object(builder,"min_label"));
	gtk_label_set_text((GtkLabel*)min_label,"-");
	gtk_label_set_markup(GTK_LABEL(min_label),"<span foreground='white' background='#B0D047' font_desc='20'>-</span>");

	//*show_right_event_box;
	show_right_event_box = GTK_WIDGET(gtk_builder_get_object(builder,"show_right_event_box"));
	g_signal_connect(G_OBJECT(show_right_event_box),"button-press-event",G_CALLBACK(show_right_event),NULL);
...
GtkWidget *create_main_window(){
    ...
    gtk_builder_add_from_file(builder, "wether_gtk_fixed.glade", NULL);//读取glade文件
	...
	sprintf(image_file_str,"res/%d.png",wether_type);
	...
	sprintf(tmp_str,"<span foreground='#4A4C4D' font_desc='12'>紫外线强度：%s</span>",
				wether_now_info+4);
